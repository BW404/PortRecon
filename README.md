...
├── 2.ollama-checker.py        # Stage 2: Ollama service verification (produces `ollama.csv`)
├── 3. ollama-model-list.py    # Utility: query Ollama hosts and export models (reads `ollama.csv` by default)
...
├── ollama.csv                 # Final results from `2.ollama-checker.py` (generated)
├── ollama_hosts.csv           # Model-enriched CSV (generated by `3. ollama-model-list.py`)
├── ollama_hosts.json          # Full JSON per-host results (generated by `3. ollama-model-list.py`)
├── ollama_hosts_with_models.json # Filtered JSON with only hosts that returned models (generated by `3. ollama-model-list.py`)
...
## Ollama Model List Helper (optional)

A small standalone helper script (`3. ollama-model-list.py`) that queries a list of hosts for Ollama models and exports results in JSON and CSV formats. It is intended to run after `2.ollama-checker.py` and by default reads the CSV produced by that step (`ollama.csv`). The helper also accepts a plain hosts file (one host per line) or other CSVs with a `Host` column.

Basic usage:

```bash
# Default: reads `ollama.csv` (output from 2.ollama-checker.py) and writes three outputs
python "3. ollama-model-list.py"

# Or specify a different input file (plain hosts file or CSV)
python "3. ollama-model-list.py" --hosts hosts.txt

# Specify custom output names
python "3. ollama-model-list.py" --output-json ollama_hosts.json --output-csv ollama_hosts.csv --output-filtered ollama_hosts_with_models.json
```

By default the script writes:
- `ollama_hosts.json` — full per-host JSON results
- `ollama_hosts.csv` — CSV summary with columns `host,status,runtime,models,error`
- `ollama_hosts_with_models.json` — filtered JSON containing only hosts that returned one or more models

Notes:
- The script accepts bare host entries (`host:port`) or full URLs (`http://host:port` or `https://host:port`).
- Run `2.ollama-checker.py` first to generate `ollama.csv`, then run `3. ollama-model-list.py` to enrich and export the model lists.
...
| `ollama_hosts.csv` | Model-enriched CSV | `3. ollama-model-list.py` |
| `ollama_hosts.json` | Full JSON per-host results | `3. ollama-model-list.py` |
| `ollama_hosts_with_models.json` | Filtered JSON with only hosts that returned models | `3. ollama-model-list.py` |
